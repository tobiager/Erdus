...
export function oldToNew(oldDoc: OldDoc): NewDoc {
  const shapes = oldDoc.shapes.filter(s => s.type === 'Table');

  // 1) tablas → nodos
  const nodes: NewNode[] = shapes.map(s => {
    const d = s.details;
  ...
  // 2) FKs → edges (agrupando compuestas por padre y orden fkSubIndex)
  type Group = { child:number; parent:number; parts:{ attr:OldAttr; idx:number }[] };
...
export function newToOld(newDoc: any): OldDoc {
  ...
  // 2) edges → FKs + connectors (orden mantiene fkSubIndex)
